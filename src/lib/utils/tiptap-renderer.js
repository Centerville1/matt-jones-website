...
async function renderNode(node, enableSyntaxHighlighting = true) {
  ...
  const childrenHtmlArray = await Promise.all(
    content.map(
      /** @param {any} child */ (child) =>
        renderNode(child, enableSyntaxHighlighting),
    ),
  );
  ...
  switch (node.type) {
    ...
    case 'imageResize':
      ...
      const resizeTitle = node.attrs?.title
        ? ` title="${escapeHtml(node.attrs.title)}"`
        : '';
      const resizeWidth = node.attrs?.width
        ? ` width="${node.attrs.width}"`
        : '';
      const resizeHeight = node.attrs?.height
      ...
      if (wrapperStyle || containerStyle) {
        ...
        const containerStyleAttr = containerStyle
...
function escapeHtml(text) {
  ...
  return text.replace(
    /[&<>"']/g,
    (/** @type {string} */ char) => map[/** @type {keyof typeof map} */ (char)],
  );
}
...
function extractText(node) {
  ...
  if (node.content && Array.isArray(node.content)) {
    return node.content
      .map(/** @param {any} child */ (child) => extractText(child))
      .join(' ');
  }
