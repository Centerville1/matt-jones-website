<script>
  ...
  function confirmDelete(tag) {
    if (
      confirm(`Delete tag "${tag.name}"? This will remove it from all posts.`)
    ) {
      ...
      <tbody>
        {#each data.tags as tag (tag.id)}
          <tr>
            ...
            <td class="actions">
              <button class="btn-secondary" onclick={() => openEditModal(tag)}
                >Edit</button
              >
              <button class="btn-danger" onclick={() => confirmDelete(tag)}
                >Delete</button
              >
            </td>
          </tr>
        {:else}
          <tr>
            <td colspan="4" class="empty"
              >No tags yet. Create one to get started!</td
            >
          </tr>
        {/each}
      </tbody>
