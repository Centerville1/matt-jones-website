<script>
  ...
  function confirmDelete(series) {
    if (
      confirm(
        `Delete series "${series.title}"? Posts in this series will not be deleted.`,
      )
    ) {
        ...
        {#each data.series as series (series.id)}
          <tr>
            ...
            <td class="actions">
              <button
                class="btn-secondary"
                onclick={() => openEditModal(series)}>Edit</button
              >
              <button class="btn-danger" onclick={() => confirmDelete(series)}
                >Delete</button
              >
            </td>
          </tr>
        {:else}
          <tr>
            <td colspan="6" class="empty"
              >No series yet. Create one to get started!</td
            >
          </tr>
        {/each}
      </tbody>
    ...
    <div class="modal" onclick={(e) => e.stopPropagation()}>
      ...
      >
        ...
        <div class="form-group">
          ...
          <small
            >URL-friendly identifier (auto-generated from title if left blank)</small
          >
        </div>
