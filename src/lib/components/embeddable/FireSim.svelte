<script>
	...
	$: if (contextStore && $contextStore) {
		if ($contextStore.gridHeight !== undefined) _gridHeight = $contextStore.gridHeight;
		if ($contextStore.gridWidth !== undefined) _gridWidth = $contextStore.gridWidth;
		if ($contextStore.scale !== undefined) _scale = $contextStore.scale;
		if ($contextStore.treeChance !== undefined) _treeChance = $contextStore.treeChance;
		if ($contextStore.treeSprout !== undefined) _treeSprout = $contextStore.treeSprout;
		if ($contextStore.nearNeighborSprout !== undefined) _nearNeighborSprout = $contextStore.nearNeighborSprout;
		if ($contextStore.treeGrow !== undefined) _treeGrow = $contextStore.treeGrow;
		if ($contextStore.burnRepair !== undefined) _burnRepair = $contextStore.burnRepair;
		if ($contextStore.houseChance !== undefined) _houseChance = $contextStore.houseChance;
		if ($contextStore.spontaneousCombustion !== undefined) _spontaneousCombustion = $contextStore.spontaneousCombustion;
		if ($contextStore.fireSpreadChance !== undefined) _fireSpreadChance = $contextStore.fireSpreadChance;
	...
	$: if (contextStore && $contextStore && $contextStore.resetTrigger !== undefined) {
			...
			for (let y = 0; y < realHeight; y++) {
				for (let x = 0; x < realWidth; x++) {
					...
					if (color === BACKGROUND.toString()) {
						...
						if (Math.random() < _treeSprout + aliveNeighbors * _nearNeighborSprout) {
						...
						if (Math.random() < burningNeighbors * _fireSpreadChance) {
							...
							} else if (Math.random() < burningNeighbors * (_fireSpreadChance / 2)) {
								modifiedMatrix[y][x] = FIRECOOL;
							}
...
<div class="fire-sim-container">
	...
	<div class="legend">
		<div class="legend-item">
			<span class="legend-box" style="background-color: rgb(51, 31, 14);"></span>
		...
		<div class="legend-item">
			<span class="legend-box" style="background-color: rgb(255, 126, 42);"></span>
			<span class="legend-label">Fire (Cool)</span>
		</div>
		<div class="legend-item">
			<span class="legend-box" style="background-color: rgb(220, 50, 0);"></span>
		...
		<div class="legend-item">
			<span class="legend-box" style="background-color: rgb(80, 130, 130);"></span>
			<span class="legend-label">Burnt Ground</span>
		</div>
		<div class="legend-item">
			<span class="legend-box" style="background-color: rgb(100, 100, 100);"></span>
