<script>
  ...
  let structuredData = $derived({
    ...
    ...(data.post.readTimeMinutes && {
      timeRequired: `PT${data.post.readTimeMinutes}M`,
    }),
    ...(data.series && {
      isPartOf: {
        '@type': 'Series',
...
<svelte:head>
  ...
  <meta
    ...
    content={data.post.publishedAt
      ? new Date(data.post.publishedAt).toISOString()
      : new Date(data.post.createdAt).toISOString()}
  />
  <meta
    property="article:modified_time"
    content={new Date(data.post.updatedAt).toISOString()}
  />
  <meta
    property="article:author"
    content={data.post.authorName || 'Matt Jones'}
  />
  ...
  <article class="blog-post">
    ...
    <div class="post-body">
      <BlogPostContent
        ...
        backgroundPattern={data.post.background || ''}
      />
    </div>
